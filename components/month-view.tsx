    ...
    <div className="h-full">
      ...
      <div className="grid grid-cols-7 border-b dark:border-gray-700">
        {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((day) => (
          <div key={day} className="p-4 text-center border-r dark:border-gray-700 last:border-r-0">
            <div className="text-sm font-medium text-gray-500 dark:text-gray-400">{day}</div>
          </div>
        ))}
      </div>
                ...
                <div className="space-y-1">
                  {dayClasses.slice(0, 3).map((classEvent) => (
                    <div
                      ...
                      title={`${classEvent.title} - ${classEvent.location}`}
                    >
                      <div className="font-medium truncate">{classEvent.title}</div>
                      <div className="text-xs opacity-75">
                        {new Date(classEvent.startTime).toLocaleTimeString("en-US", {
                          hour: "numeric",
                          minute: "2-digit",
                          hour12: true,
                        })}
                  ...
                  {dayClasses.length > 3 && (
                    <div className="text-xs text-gray-500 dark:text-gray-400 pl-1">+{dayClasses.length - 3} more</div>
                  )}
                </div>
              </div>
            </div>
